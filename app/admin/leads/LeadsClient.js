// app/admin/leads/LeadsClient.jsx
'use client';
...
export default function LeadsClient({ leads }) {
  ...
  function parseStart(d){ if(!d) return null; const [y,m,day]=d.split('-').map(Number); return new Date(y,(m||1)-1,day||1,0,0,0,0); }
  function parseEnd(d){ if(!d) return null; const [y,m,day]=d.split('-').map(Number); return new Date(y,(m||1)-1,day||1,23,59,59,999); }
  ...
  const filtered = useMemo(() => {
    ...
    return leads.filter(l => {
      ...
      const blob = [
        l.name, l.email, l.phone, l.projectType, l.budget, l.timeline, l.message, created.toLocaleString()
  ...
  const sorted = useMemo(() => {
    const [key, dir] = sort.split('-'); const mul = dir === 'asc' ? 1 : -1;
    ...
    <main className="mx-auto max-w-7xl px-4 py-10">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <form action="/admin/logout" method="post">
          <button className="rounded-[5px] border border-white/15 px-3 py-2 text-base hover:bg-white/5">Logout</button>
        </form>
      </div>
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-8">
        ...
        <div className="flex items-center gap-2 md:col-span-3">
          ...
          <button
            type="button" onClick={()=>{setFromDate(''); setToDate('');}}
